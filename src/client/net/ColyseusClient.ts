...
export class ColyseusGameClient {
  ...
  private handleSnapshot(snap: SnapshotMessage): void {
    ...
    // mutates the incoming wire message (we own it post-decode) so
    // downstream `snap.states` reads see the local view — same effect
    // as the legacy `snap = { ...snap, states: ... }` clone but with
