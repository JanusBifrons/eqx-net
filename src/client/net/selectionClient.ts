 ...
 * STRUCTURE selections use the channel — drones read health from the
 ...
 *  drone (those never use the channel). For a structure the wire `id` is
...
export function toSelectWire(
  ...
  return null; // drone — no server channel
}

/** Start the server stats stream for a ship/structure selection. No-op (returns
 *  false) for drone or when there's no live room. */
