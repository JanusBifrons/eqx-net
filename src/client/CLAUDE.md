...
- **Advanced / hidden UI lives in the right-edge `Drawer`** ([layout/Drawer/AdvancedDrawer.tsx](layout/Drawer/AdvancedDrawer.tsx)) with a vertical icon-only tab rail. **The drawer must NOT be a `SwipeableDrawer` and `keepMounted` must stay OFF** — both regressed mobile RTT from ~50 ms to ~2.4 s in measurement, because (a) SwipeableDrawer attaches global touch listeners that fire on every joystick movement, and (b) keepMounted causes tab content (`ConnectionDiagnostics`, `DevOverlay`, `LogPanel`) to re-render at the snapshot rate (~17 Hz) even when invisible. Adding a new tab: append a `TabSpec { id, label, icon, node, bottom? }` to the `TABS` array in `AdvancedDrawer.tsx`. Tab catalogue today: `galaxy` (top + default-selected), `profile`, `settings`, `debug` (last is sticky-bottom). `isDrawerOpen` and `drawerTab` are discrete UI flags in Zustand, purity-clean. The drawer toggle (`<DrawerToggle>`) is rendered into the `top-right` slot.
