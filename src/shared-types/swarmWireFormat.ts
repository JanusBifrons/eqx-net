 ...
 *   [+28] u8   shipKind      (v2: index into `SHIP_KINDS_LIST`; meaningful only when kind=drone,
 *                             0 / ignored otherwise. Drives drone silhouette + colour on the
 *                             client renderer. Kinds may only be APPENDED to the catalogue —
 *                             reordering invalidates this index for in-flight v2 packets.)
 *
 ...
 * clients reading v2 with the v1 stride mis-attribute every byte after the
 * first record — never silently fall back. v1 had a 28-byte record without
 * the trailing `shipKind` byte.
 */

export const SWARM_WIRE_VERSION = 2;

export const SWARM_HEADER_BYTES = 8;
export const SWARM_RECORD_BYTES = 29;
/** Offset of the `shipKind` byte within a record (v2). Index into `SHIP_KINDS_LIST`. */
export const SWARM_REC_SHIP_KIND_OFF = 28;
