...
- **`CONTACT_BATCH { tick, contacts: Contact[] }`** — emitted per tick when Rapier's `EventQueue` produced contact-force events above `CONTACT_FORCE_FLOOR` (Stage 2 of the network-feel roadmap). Each `Contact` carries `{ aId, bId, vAxPost, vAyPost, vBxPost, vByPost, forceMagnitude, impactSpeed? }`. `impactSpeed` (2026-06-07) is the closing speed at impact — magnitude of the relative velocity from the bodies' PRE-step velocities (the worker keeps a `prevVel` snapshot = last tick's post-step velocities and passes it to `drainContacts`). It is the input to the IMPACT-SPEED-GATED ramming-damage model (`core/combat/Ramming.ts` `aggregateRamming` → `ramDamageFromImpactSpeed`): a collision below `RAM_MIN_IMPACT_SPEED` deals 0 damage so a slow drift / docking tap into a structure is free. Post-step velocity is NOT a usable impact measure (a ship that head-on-stops against a static structure has `vPost≈0` yet a hard impact). The main thread re-emits each as `COLLISION_RESOLVED` on the Bus and broadcasts a `collision_resolved` network message. Skipped (no message) when the tick produced no qualifying contacts.
