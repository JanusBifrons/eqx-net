...
- **In-sector squad herding — leader + FLOCKING/boids (2026-06-18, supersedes the WS-4 wedge-slot scheme)**: an IDLE, unassigned squad gathered in one sector flies as a leader-led FLOCK. `LivingWorldDirector.flockStep()` (called from `tick()` after `roamStep`) designates a LEADER (first active member in `botIds` order) given a wandering in-sector COURSE (a point on the radius-`FORMATION_DEST_RANGE` ring roughly OPPOSITE the leader's current bearing ± `FORMATION_LEG_SPREAD`, so every leg is LONG and the leader genuinely CRUISES instead of milling near a randomly-close point), and marks every OTHER member a FOLLOWER via the new `LivingWorldRoom.setBotFlockFollow(botId, leaderId, memberIds)` hook. **The director only assigns ROLES at the ~1.5 s control cadence; the STEERING is per-tick (60 Hz) in the drone brain** — which is the whole point (the old wedge-slot scheme pushed a STATIC slot every 1.5 s and `arrive`d/STOPPED at it, so it read as a static blob, not a formation). `HostileDroneBehaviour.tickFlock` runs continuous boids against LIVE poses each tick: COHESION toward the leader, SEPARATION from squad neighbours (cluster AROUND the leader at a spacing, don't pile on), ALIGNMENT with the leader's heading (the herd flies the course as one). The follower reads its leader's + neighbours' live poses via the new **server-only `AiWorldView.resolveEntityInto(id, out)` seam** (wired to `SectorRoom.swarmEntityPoseInto`, a SAB read into a caller-owned buffer — NOT the shared `swarmEntitySnapshot` scratch that `self` aliases). **BOOST (drone analogue of a player holding boost)**: a follower farther than `FLOCK_FOLLOW_DISTANCE × FLOCK_BOOST_GAP_FACTOR` from its leader applies the kind's REAL player-boost impulse (`thrustImpulse × boostMultiplier`) to catch up — without it, the drone AI cruise (`ai.thrust`, far below a player's) can't close on a moving leader and the herd strings out (the user: "why can't drones just boost like players?"). In formation it eases back to the calm `ai.thrust` roam cruise. All SERVER-ONLY (no client brain → no lockstep surface, no wire bump); pure boids math in [src/core/ai/flocking.ts](../core/ai/flocking.ts) (alloc-free, accumulator + caller-owned out). COMBAT overrides it (a waved/hostile drone pursues). The wedge-slot helpers in `src/core/ai/formation.ts` are now unused (kept for a future rigid-formation mode). **Netgate** required (AI tick + population motion brush the swarm broadcast — invariant #8). Locks: `flocking.test.ts`, `HostileDroneBehaviour.flock.test.ts`, `tests/integration/sectorRoom/livingWorldFormation.test.ts` (squad flies as a group AND converges, leader+7 followers). **Tuning (the `FLOCK_*` weights/radii + `FORMATION_DEST_RANGE/ARRIVE/LEG_SPREAD`) is a feel knob — confirm on-device.**
