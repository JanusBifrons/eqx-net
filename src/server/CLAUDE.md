...
- **In-sector squad formation (Phase 5 WS-4, 2026-06-14 — "roaming bots just sit there… make a formation and fly in formation")**: an IDLE, unassigned squad gathered in one sector now FLIES IN FORMATION toward arbitrary in-sector A→B destinations instead of each drone orbiting the origin independently. `LivingWorldDirector.formationStep()` (called from `tick()` after `roamStep`) designates a leader (first active member in `botIds` order), picks/refreshes a random in-sector destination (`FORMATION_DEST_RANGE`, re-picked within `FORMATION_DEST_ARRIVE` of the leader), and assigns each follower a **wedge slot** (`formationSlotWorldPose` in [src/core/ai/formation.ts](../core/ai/formation.ts), rotated into the leader's live frame so the wedge faces travel) via the new `LivingWorldRoom.setBotMoveTarget(botId,x,y)` hook + `getBotPose(botId)` (SAB pose). The drone's IDLE `HostileDroneBehaviour` flies to its target via the pure `arrive` ([src/core/ai/steering.ts](../core/ai/steering.ts)) — ramping thrust to 0 within `MOVE_ARRIVE_SLOW_RADIUS` so per-kind damping brakes it to a STOP at the slot (the "slow down and come to a stop, don't float past" feel). The move target rides the optional `IAiBehaviour.setMoveTarget?/clearMoveTarget?` seam — **SERVER-ONLY** (the client never ticks the drone brain → no lockstep surface, no wire bump). COMBAT overrides it (a waved/hostile drone ignores the move target and pursues). Because the squad stays clustered in formation, it gathers + spools + warps between sectors as ONE unit (the roam hop in `roamStep` only fires once gathered, and `advanceMembersTowardGoal` starts every member's transit in the same control tick). **Netgate** required (AI tick + population motion brush the swarm broadcast — invariant #8). Locks: `steering.test.ts`, `formation.test.ts`, `HostileDroneBehaviour.moveTarget.test.ts`, `tests/integration/sectorRoom/livingWorldFormation.test.ts` (a gathered squad gets 8 DISTINCT formation targets). **Tuning (`FORMATION_DEST_RANGE/ARRIVE/SPACING`, `MOVE_ARRIVE_SLOW_RADIUS`) is a feel knob — confirm on-device.**
