...
- **In-sector squad herding — leader + FLOCKING/boids (2026-06-18, supersedes the WS-4 wedge-slot scheme; boost REMOVED, spawn-clustering ADDED)**: an IDLE, unassigned squad gathered in one sector flies as a leader-led FLOCK. `LivingWorldDirector.flockStep()` (called from `tick()` after `roamStep`) designates a LEADER (first active member in `botIds` order) given a wandering in-sector COURSE (a point on the radius-`FORMATION_DEST_RANGE` ring roughly OPPOSITE the leader's current bearing ± `FORMATION_LEG_SPREAD`, so every leg is LONG and the leader genuinely CRUISES instead of milling near a randomly-close point), and marks every OTHER member a FOLLOWER via `LivingWorldRoom.setBotFlockFollow(botId, leaderId, memberIds)`. **The director only assigns ROLES at the ~1.5 s control cadence; the STEERING is per-tick (60 Hz) in the drone brain** — which is the whole point (the old wedge-slot scheme pushed a STATIC slot every 1.5 s and `arrive`d/STOPPED at it, so it read as a static blob, not a formation). `HostileDroneBehaviour.tickFlock` runs continuous standard boids against LIVE poses each tick: COHESION toward the leader with an ARRIVAL ramp (the pull fades to 0 as the follower nears `FLOCK_FOLLOW_DISTANCE` — "slow down once close", no overshoot), SEPARATION from squad neighbours (cluster AROUND the leader at a spacing, don't pile on), ALIGNMENT with the leader's heading (the herd flies the course as one). The follower reads its leader's + neighbours' live poses via the **server-only `AiWorldView.resolveEntityInto(id, out)` seam** (wired to `SectorRoom.swarmEntityPoseInto`, a SAB read into a caller-owned buffer — NOT the shared `swarmEntitySnapshot` scratch that `self` aliases).
  ...
  - **VISIBILITY — a roaming herd re-forms CENTRAL + hops RARELY (2026-06-19, live-diagnosis fix).** The flock math + clustering above were correct, but in live play the herd was never SEEN. Live `flock_debug` (the new env-gated diag, below) proved it: a squad spawns at the ~4600 u EDGE, the leader course targeted a radius-2000 ring CENTRED ON ORIGIN, and the ~45 s roam dwell was SHORTER than the edge→centre cruise — so the herd perpetually orbited the OUTER ring (leaders pinned at radius ~3600–4100) at/beyond a central player's interest (~3072 u) + view (~±650 u @ zoom 1 / ±1600 @ min zoom). Three changes: **(1)** a ROAMING squad's hop ARRIVAL re-forms clustered in the central, player-visible zone via `squadCentralPose` (radius ~450), threaded through `HunterBotWarpController.arrivalPoseFor` which the director supplies ONLY for roaming squads (`targetFactionId === null`); WAVE hops + from-nowhere ingress keep the EDGE pose (the entry-only-ingress invariant binds INGRESS, NOT graph-traversal hop arrivals — `bot_transit_commit` may land anywhere). **(2)** `FORMATION_DEST_RANGE` 2000 → 1000 so the leader patrols the central ring. **(3)** `roamIntervalMs` 45 s → 6 min: roamers LINGER + flock A→B in a sector, then drift; frequent hopping is for WAVES bearing down on a base, not roamers. Env-gated `flock_debug` (`EQX_FLOCK_DEBUG=1`, one event per idle squad per control tick → `GET /dev/events`: skip reason or `leaderR`/`maxGap`/`gathered`/`dest`) is the herd diagnostic + tuning lever. Verified live: leaderR ~3600 → ~700, herd tight (`maxGap` ~130) AND visible next to a player at origin. **`squadEdgePose` (edge/ingress) is UNCHANGED; only roam ARRIVAL diverges to central.** 🔴 netgate. **DEFERRED follow-up (the user's "no warp until cohesive"):** a cohesive SYNC-hop primitive — gather-until-cohesive → warp the whole squad together → regroup → repeat — for both roam (one rare hop) + waves (frequent group-up-then-hop toward the base); touches the wave pipeline, its own PR + netgate.
  - All SERVER-ONLY (no client brain → no lockstep surface, no wire bump); pure boids math in [src/core/ai/flocking.ts](../core/ai/flocking.ts) (alloc-free, accumulator + caller-owned out). COMBAT overrides it (a waved/hostile drone pursues). The wedge-slot helpers in `src/core/ai/formation.ts` are now unused (kept for a future rigid-formation mode). **Netgate** required (AI tick + population motion brush the swarm broadcast — invariant #8). Locks: `flocking.test.ts` (arrival ramp + separation + cohesion), `HostileDroneBehaviour.flock.test.ts` (no boost / thrust capped at cruise, leader throttle, leader-hold), `population.test.ts` (`squadEdgePose` clusters a squad ≪ a sector-wide scatter; `squadCentralPose` clusters CENTRAL not edge), `tests/integration/sectorRoom/livingWorldFormation.test.ts` (gathered squad clustered `gap0<1500` + tight `gap1<400` + alive; AND a ROAMING squad re-forms CENTRAL — `maxRadius<1500` not the ~4600 edge — on hop arrival). **Tuning (`FLOCK_*` weights/radii, `LEADER_CRUISE_THROTTLE`, `FLOCK_GATHER_RADIUS`, `SQUAD_SPAWN_*_JITTER`, `SQUAD_ARRIVAL_CENTRAL_RADIUS`, `FORMATION_DEST_RANGE/ARRIVE/LEG_SPREAD`, `roamIntervalMs`) is a feel knob — confirm on-device.**
