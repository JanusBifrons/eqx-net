...
## 2026-05-19 — The combat diagnostic funnel was half-blind, and it blocked repro-first on two other bugs

Smoke capture `76idw1` recorded `bot_respawn` + `shield_broken` (a kill and damage demonstrably happened) yet **zero** `damage`/`hit_ack`/`destroy` events in any ndjson — only `fire`/`fire_received`. Invariant #13 (failing test first, from the user's capture) was therefore *impossible* to satisfy for the inconsistent-damage and explosion reports: the evidence those bugs live in was never recorded.

1. **A diagnostic gap is a force-multiplier bug — fix it before the bugs it hides.** Two separate user-reported issues were un-reproducible not because they were hard but because the instrument didn't capture their evidence class. The keystone-first ordering (extend the capture, *then* chase the bugs) was correct and is the general pattern: when repro-first stalls, suspect the instrument before the hypothesis.

2. **The gap was two-part and a test at one level would have missed the other.** `damage`/`hit_ack`/`destroy` were *never emitted* to the server event ring (a `SectorRoom` emit gap); `shield_broken`/`shield_restored`/`ram_damage` *were* emitted but `routeBucket` sent them to `other` (a `diagRouter.BUCKETS` table gap). Locking only the routing table (unit) would pass while the funnel stayed blind; locking only the live emit (integration) would pass with the wrong bucket. Both tests, both levels — the recurring "write the test where the bug lives, and if there are two levels, write two."

3. **Single-ownership turned a 4+4-site sprinkle into a structural guarantee.** Rather than 8 adjacent `serverLogEvent` calls (divergence-prone, and one inevitably forgotten next time a branch is added), all damage/destroy broadcasts now route through `broadcastDamage`/`broadcastDestroy` (broadcast + log in one place). The diag-suppressed kill path (`evictSwarmEntity{broadcast:false}`) simply never calls them, so "don't log phantom LoadShedder evicts" needs no flag — it falls out of the structure. Prefer a chokepoint that *can't* be bypassed over N correct-today call sites.

4. **The integration kill had to be made sim-speed-independent.** The first cut fired at `tick: serverTick` every 60 ms and asserted a destroy; it flaked because under in-process test load the sim ticks below wall-clock, so most fires cooldown-rejected and the scout (90 shield + 90 hull) never died. Claiming monotonically-advancing ticks (`base + i*12`, always > the 10-tick cooldown, future-claims pass temporal plausibility unchanged) makes every shot accepted regardless of sim speed → deterministic kill. A combat integration test that depends on wall-clock tick rate is not repeatable; drive the accept-gate by claimed tick, not by sleeping.

## 2026-05-19 — Death explosion: the obvious root cause was wrong, and the precedent test would have been the trap

Reported: "the first ship I killed didn't show the explosion effect, it just vanished." Rev-1 of the plan (and an Explore agent) confidently root-caused it as `killSwarmEntity` deleting `mirror.swarm` *before* flagging `explodingShips`, asymmetric vs `killEntity`. A hostile review demanded that be verified in the renderer; it was false.

1. **The plausible statement-order theory was a no-op, and its unit test was the canonical trap.** `killSwarmEntity` is a synchronous message handler — both mutations land before any frame; the renderer's explosion loop (`PixiRenderer.update` ~985) runs *before* the sprite cull (~1107), so the pose is resolvable regardless of intra-call order. Reordering two lines changes nothing observable, yet the proposed "assert `swarm` still has the entity when `explodingShips.add` is called" unit test would have gone GREEN — a textbook 2026-05-14-class green-while-broken lock. The real cause was `App.tsx:367-370`: `renderer.update()` every 2nd frame under the worker, `explodingShips.clear()` *every* frame → ~50% of kills wiped before render. Verifying the "obvious" fix against the actual consumer before writing its test is what caught this; the hostile-review step paid for itself.

2. **Cargo-culting the boundary-probe precedent across bug classes is the same trap one level up.** The 2026-05-14 rule ("if the bug is at the worker boundary, the test MUST cross it") is right *for clone-boundary bugs*. This bug only *manifests* through the worker (it's what makes `shouldRender` toggle) but *lives* in App.tsx main-thread loop sequencing. A damage-number-style renderer probe drives `postFrame()` itself — it replaces the App.tsx loop, so it structurally cannot observe the gate defect; it would pass while broken. Invariant #13 says test where the bug lives, not where the nearest precedent points. The faithful lock is a unit test on the extracted seam (`render/perFrameTriggers.test.ts` — `consumeOneFrameTriggers(mirror, didRender)`), asserting that the trigger set is a no-op on skip frames and a clear on render frames. Documenting *why* the probe and the pure-helper tests are both wrong-level is part of the lock — so the next person doesn't "simplify" it back into the trap. (Note: this fix was independently implemented on two branches; mobile-perf's `consumeOneFrameTriggers` seam landed; an alternate `computeFrameGate` seam shipped on `fix/wrap-up-known-issues` was superseded by it during the 2026-05-25 branch consolidation.)

3. **Couple the decision, not just the action.** The fix is one line of logic (the clear MUST be gated on the same condition as the render) but the durable form is a named seam (`consumeOneFrameTriggers`) whose invariant *is* that equality, so any future one-frame trigger added to the mirror inherits the rule instead of re-deriving (and re-breaking) it. The bug existed because two decisions that must agree (render? clear?) were written independently.

## 2026-05-19 — "Inconsistent damage": the reported number wasn't a bug, and the planned test would have proven nothing

The smoke report was "damage feels inconsistent"; capture `76idw1` showed 133 client `fire` vs ~28 server `fire_received`. Rev-1 of the plan proposed an integration test asserting "every accepted on-target fire yields exactly one DamageEvent with `damage===hit_ack.damage`." The hostile review killed it; both halves were wrong.

1. **The proposed lock duplicated an already-GREEN test and asserted a false invariant.** `hitAckContract.test.ts` already drives the real fire→hit→damage pipeline and locks `hit_ack.damage===DamageEvent.damage`; the new test would have been GREEN on day one (reproducing nothing). Worse, "exactly one DamageEvent per accepted fire" is false: `handleFire` calls `applyDamage` per-mount (each broadcasts its own `damage`), `hit_ack` is aggregate-once, and asteroids emit `hit_ack{hit:true}` with no `damage` at all. A test that encodes a false invariant is worse than no test — it fails for the wrong reason or passes while hiding the truth. Before writing a "repro," check whether an existing test already covers the path and whether the asserted invariant is actually true of the system.

2. **The "anomaly" was the rate-limiter, and the right move was to make it VISIBLE and LOCKED, not to "fix" it.** A client may emit a fire intent every input tick; the per-shooter cooldown accepts one per 10-tick window. `fire_received` was logged *after* the cooldown early-return, so rejected shots left no trace — the 133→~28 gap *looked* like lost damage but is the by-design anti-rapid-fire guard. Phase 1 made the rejected `hit_ack` visible; Phase 4 extracted the decision to a pure `isFireOnCooldown` and characterisation-locked the accounting (133@Δ1 → exactly 14 accepts; first-always-accepted + no-two-accepts-closer-than-cooldown invariant). No behaviour shipped. When a "bug report" is really a misunderstanding of correct behaviour, the deliverable is instrumentation + a lock that *explains* it, not a speculative change to a security-sensitive guard.

3. **A characterisation closed-form is a liability; lock the invariant instead.** The first cut asserted `accepts === 1 + floor((n-1)·s / cooldown)` for every sub-cooldown spacing — wrong for spacings that don't divide the cooldown (greedy 1-per-window ≠ that formula at s≥6). The robust lock is the *property* the rate-limiter guarantees — first claim accepted, no two accepts closer than `cooldown`, accepts < claims — plus one specific hand-verified number for the headline case. Re-deriving an algorithm's output with a parallel formula in its own test just adds a second thing that can be wrong.

4. **Freeze a known-bad behaviour explicitly as the deferred fix's baseline.** The cooldown's reorder weakness (an out-of-order earlier fire is dropped) is real but security-sensitive and not yet repro'd from a capture. The lock includes a test that *characterises* today's wrong-under-reorder output, labelled as the baseline the repro-gated follow-up must start from — not as desired behaviour. This keeps the deferral honest: the weakness is documented and pinned, so the eventual fix is a visible, reviewed delta, not a silent semantics change to the only anti-rapid-fire guard.

## 2026-05-19 — "Respawn lag": a memory-note hypothesis falsified by reading the code, and the honest two-bucket close

A persisted memory note carried "respawn lag" as task #14: the rejoin path supposedly lacked the `resetPredictionState()` hardening transit got, so a poisoned `_rttWelford`/saturated `leadTicks` survived a respawn and caused a 3–4 s stall. The plan's Phase 2 (rev-1) was built on it. Tracing the actual respawn path killed the hypothesis.

1. **A recalled memory is a hypothesis, not a finding — verify it against current code before building on it.** The Phase-5 UI does not call the in-place `respawnShip` RPC at all (App.tsx says so; grep: zero UI call sites). Death → galaxy-map → re-pick runs `game→connecting→game`, which unmounts/remounts `GameSurface` and constructs a **brand-new `ColyseusGameClient`**. No instance survives a respawn, so the poisoned-state class the note feared is *structurally impossible*. The memory described a real risk for the *transit* path (same instance, hot-swap) and a dead RPC — not the live respawn path. Memory entries age; the code is ground truth. (The harness even surfaces this: `respawnShip`/`handleRespawnAck` are engineering/test-only.)

2. **"Falsified" must not collapse into "nothing there" — separate the buckets explicitly.** Respawn *does* have a real, perceptible cost: a 200 ms phase-flip + GameSurface remount + fresh `joinOrCreate` + Pixi re-init + the deliberate 5 s WarpScreen floor. That is *correct by design* (a full room re-join), not the poisoned-prediction bug. The honest close documents both — (a) the feared residual is CLOSED (impossible), (b) the by-design cost exists and is not patched — so a future reader doesn't mistake the silence for "we never looked," and a future "respawn feels slow" report is correctly scoped as a separate optimisation with its own repro, not a re-litigation of this.

3. **Closing a falsified bug is a docs deliverable, not a no-op.** No code, no test (no behaviour change, no on-device repro — the anti-speculative discipline). The artefact is the falsification itself, written where the next person will look (the `Sector handoff` section, beside the transit rule it was confused with) plus this lesson. An un-recorded "we decided it was fine" is indistinguishable from "we forgot."

## 2026-05-19 — `pnpm bench` was a half-hollow rung: vitest 2.x bench mode silently skips suite hooks

Invariant #8 lists `pnpm bench` as a green-bar. It exited 0 while 14 of 23 benchmarks emitted `NaNx faster than` — zero samples. Memory said "0 samples for all" (2026-05-17); a hostile reviewer said "7 M samples, fine." Both wrong: I ran it. The pure-sync micro-benches (spring, weapon-hittest) sampled fine; the perf-budget-relevant suites (physics-tick, swarm-broadcast, persistence-worker — exactly the ones that guard the 16.67 ms tick) were hollow.

1. **Verify a disputed premise yourself; the truth is usually neither extreme.** Two confident, contradictory secondhand claims. The five-minute `--outputJson` run settled it precisely and rescoped the phase from "fix 0-sample bug" / "drop, doesn't exist" to "restore exactly the three hook-dependent suites." Reproduce-or-drop, where "reproduce" is *you*, not a recollection.

2. **The split was a perfect predictor: hollow ⟺ has a `beforeAll`.** vitest 2.x runs `bench()` via tinybench but does NOT execute `beforeAll`/`beforeEach`/`afterAll` for benchmark suites. Any bench whose body reads hook-initialised state throws every iteration → tinybench records no successful run → NaN hz/mean → the silent `NaNx`. spring/weapon-hittest have no hooks (inline/module setup) and were always honest; the other three were hollow from the day the hook was added. (swarm-broadcast's hook is *sync* and still skipped — it's hook-skip, not an async-await race.)

3. **Fix the harness, never the measured body.** Setup moved to module load (top-level `await` for the async `PhysicsWorld.create()` / worker bundle; a plain block for the sync one); the `bench()` bodies are byte-identical so the numbers remain comparable to history. `afterAll` also never runs, so the persistence worker is `unref()`'d + torn down from a sync `process.on('exit')` — an un-unref'd live worker_thread would hang the bench process at the end.

4. **A green that can be hollow needs a guard that parses the result, not the exit code.** `pnpm bench` exiting 0 is meaningless when a suite can silently produce nothing. `scripts/check-bench-samples.mjs` (`pnpm bench:check`) runs the suite to `--outputJson` and fails on any zero-sample / non-finite benchmark — the invariant-#8 rung now self-checks. (It also taught its own lesson: `execFileSync('pnpm.cmd', …)` silently fails on Windows without `shell:true`; a shell-string `execSync` is the portable form.)

## 2026-05-19 — Parked local-beam frame mismatch: a reverted tangent is safe to redo *if you reconstruct WHY it was reverted*

`updateLiveBeam` hit-tested the local beam from raw `predWorld.getShipState(localId)` while the renderer draws it from the lerp-included `mirror.ships` pose — under an active reconcile offset the beam visibly misses. A prior `localBeamRay` helper attempt had been reverted (uncommitted). The hostile review flagged "flying blind into a known-abandoned approach."

1. **Step 0 — find the revert reason BEFORE re-deriving.** `git log -S` was empty and `docs/LESSONS.md` had no entry; the only record (a memory note) said it was reverted *to avoid shipping a non-reported tangent* — a SCOPE decision, not a technical defect. That distinction is the whole gate: a revert-for-scope is safe to redo as its own unit; a revert-for-a-technical-landmine would need the landmine mapped first. Don't re-attempt a reverted approach until you can say which kind it was.

2. **It's the same bug as `0e24448`, mirrored.** `0e24448` fixed the drone-TARGET side (aim the drawn pose, not the raw/ahead one); this is the local-ship-ORIGIN side of the identical disease. Recognising it as a known pattern (read the DRAWN pose, resolve once, accept the ≤1-frame "render the past") meant reusing the established `LocalBeam.ts` pure-seam shape rather than inventing — and inheriting its already-reasoned residual analysis instead of relitigating it.

3. **Presentation-only changes still get the full discipline, and the "do not revert" sign.** Even though the server stays hit-authoritative and this was explicitly *not* a reported symptom (lowest priority, user-opted-in), it shipped with a pure resolver, a geometry lock + a raw-vs-rendered divergence characterisation test, and an explicit "do NOT revert to a raw-predWorld ray" line in `src/client/CLAUDE.md` — because the thing most likely to undo it is a future reader who doesn't know it was deliberate (the same defect class that created it).
