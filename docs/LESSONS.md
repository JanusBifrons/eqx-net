...
1. **MUI's `persistent`/`docked` Drawer sets NO z-index on its Paper** (only the `temporary`/Modal variant does). So `PaperProps={{ sx:{ zIndex } }}` is the only lever, and without it the Paper is `z-auto`.
2. **`z-index` only ranks within the SAME stacking context.** The HUD slot hosts are body-level `position:fixed; z-index:Z.mobileControls(15)`. The drawer is nested inside `GalaxyPickerChrome`'s `position:fixed; z-auto` root (a stacking context). A z-1200 Paper *inside* a z-auto context can NEVER beat a body-level z-15 sibling — the whole chrome subtree ranks at the root's z-auto. Raising the Paper's z, or even the chrome root's z, doesn't help while the chrome itself is trapped under GameSurface.

Why it was confusing: it paints fine (later DOM + higher local z wins paint), so screenshots looked correct; only the hit-test (which walks the real stacking order) exposed it. Landing-screen siblings (`single-player`, z3, INSIDE the same chrome context) DID lose to the drawer — masking the body-level case.

Fix: **`<Portal>` the drawer to `document.body`** so its Paper z (Z.drawer=1200) lives at the body level, above the z-15 HUD hosts — works in both landing + in-game-warp contexts. (Also: re-enable `pointerEvents:auto` on the Paper, since the chrome root is `pointer-events:none`; and hide the landing chrome while a sector is selected, which is also better UX.) **Rule: a docked/persistent overlay that must sit above slot-anchored HUD must be Portaled to body (or rendered at body level) — a nested z-index will silently lose the hit-test even when it wins the paint.**
...
## 2026-06-12 — Ramming integration test: a big collider pre-overlapping at spawn emits NO fresh contact-force event (WS-1 / R2.31)

Writing the WS-1 ram-curve integration lock, the "light fighter into a heavy crossguard" case first timed out waiting for `ram_damage` — the event never fired. Cause: `crossguard`'s ball collider is ~213 u, so placing the two bodies at `y = ±50` (the pattern the old equal-fighter test used, where fighter balls are ~12 u and start 100 u apart) made the crossguard **already overlapping** the fighter at spawn. Rapier resolves an existing interpenetration with a positional-correction impulse, which does **not** raise a `ContactForceEvent` above `CONTACT_FORCE_FLOOR` — so `drainContacts` produced nothing and no ram damage was computed. The fix was to place the light body far enough out (`y = -400`, radius-aware) that it collides **mid-flight**, producing a genuine contact-force spike.

Lesson for ramming/collision integration tests: when one body has a large collider, do **not** SET_POSITION the pair into a pre-overlap — drive a mid-flight collision (separated by more than the sum of the radii, closing under velocity) or no contact-force event fires. The closing speed the worker measures (`Contact.impactSpeed`) is the pre-step *relative* velocity, so a stationary heavy target + a fast light attacker reads the attacker's full speed.

Second note (model change): the ramming curve is now **asymmetric mass-differential** (`ramDamageTo` = cap × reverse-square speed factor × `(mOther−mSelf)/(mOther+mSelf)` clamped) — **equal-mass collisions deal 0 at any speed**. Any future test that rams two same-kind ships together and expects ram damage is now wrong by design; use a mass differential (e.g. `fighter` vs `crossguard`) or assert `collision_resolved` instead of `ram_damage`.

## 2026-06-15 — A netgate scenario can be VACUOUS: its metrics must be provably movable by a fault in the path it claims to gate (plan: misty-teapot)

Expanding the netgate to cover the new structures + scrap features looked obvious: add a "structures-load" scenario, add a "scrap-load" scenario, reuse the existing budget, done. A hostile review caught that this would have shipped **two green gates that can never fail** — and, worse, a self-test that *falsely confirms* they work.

**Why vacuous.** Every gated netgate metric (`rollingCorrRate`, `ticksAhead`, `maxDriftUnits`, `meanDriftUnits`, `droppedSnapshotsRecent`) is a property of the **LOCAL PLAYER's prediction reconciliation** (`predictionStats.ts`). Structures and scrap are *other* entities. A real correctness regression in their wire — a wrong scrap `componentIndex`, a corrupted `structures[]` encode, a 10× payload bloat — moves **none** of those metrics:
- the `structures[]` slice carries no pose and feeds rendering, not the reconciler;
- `droppedSnapshotsRecent` detects **server-tick-sequence gaps**, and the netgate's latency proxy is **lossless + in-order by construction** (`latencyProfile.ts`) — a late or bloated payload still arrives in order, so the sequence is unbroken and the metric doesn't budge.

**The self-test trap.** The acceptance step "run `NETGATE_REGRESS=1` and confirm the scenario goes RED" feels like it proves regression power. It does not: `NETGATE_REGRESS=1` injects a worse **network profile**, which degrades the local player on *any* live scenario regardless of what structures/scrap do. A scenario whose structure encoder is completely broken still goes RED under the network inject — so the self-test reports "regression power proven" while the gate is blind to the very code it names. **A self-test must perturb the SAME path the scenario claims to gate** (here: a server-side tick-budget burn in the structures/scrap subsystem), never a generic degradation that any scenario feels.

**Resolution (the honest split).**
- **Correctness** of the structures/scrap wire → the **deterministic suite** (its proper, proxy-free layer, every PR): the encode/decode unit tests + a real-encoder→real-decoder round-trip (`tests/unit/swarmWireRoundTrip.test.ts`) + the server-chain integration tests. The two-arm latency-proxy apparatus adds nothing to a correctness check.
- **Performance under load** → the netgate, but only as the thing it *can* sense: does a heavy structures/scrap workload degrade the local player's feel via the **server tick/broadcast budget** (server falls behind → real tick-sequence gaps + climbing `ticksAhead`/drift)? That IS local-player-observable — but only if the load is heavy enough to actually stress the budget, which must be **characterised**, not assumed. So `structures-load`/`scrap-load` ship **print-only and off the per-PR path** until a server-side tick-burn fault-injection demonstrates their metrics move; only then do they get a calibrated budget and gate.

**General rule.** Before adding any gate scenario, answer: *what fault in the path I claim to gate would make this RED, and have I proven it does — by injecting THAT fault, not a proxy for it?* If you can't make it fail on a real fault in its own path, it is not a gate; it is a green light that manufactures false confidence — exactly the 2026-05-19 failure class the netgate exists to prevent. (Separately: the netgate's old hand-maintained path-filter regex omitted the structure-grid/`ScrapSpawner`/`structureKinds` server paths, so a pure structures/scrap change SKIPPED the gate entirely — a *false-negative hole*. The trigger globs now live in `tests/netgate/scenarios.ts` and CI selection is fail-closed in `select-scenarios.mjs`.)
