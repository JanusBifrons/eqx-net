...
> Still pending in Phase 3 (deferred, NO coverage loss — the specs stay intact): `respawn-cascade-input-routing` rewrite onto an engineering room + `startHostile` (the original works as a live-galaxy regression lock); `sync-health` idle-half trim; `prediction-diagnostics` T3-fold into `prediction-idle-bounded`.
