...
14. **Hot-path allocation discipline.** Any function that runs per physics tick (60 Hz), per render frame (60 Hz), per snapshot broadcast (20 Hz), or per inbound network message MUST NOT allocate on the steady path. **NEW code** in these paths needs (a) reuse of a persistent scratch / `ObjectPool` from [`src/core/util/ObjectPool.ts`](src/core/util/ObjectPool.ts), (b) an allocation regression test under `tests/integration/allocations/` asserting `pool.allocations() <= K` after a representative loop (modelled on [`src/core/combat/HitPrediction.test.ts:229`](src/core/combat/HitPrediction.test.ts) — the canonical pattern). The `/allocation-audit` skill scans the current diff for violations. Scope clarification: this applies to code added or modified in a PR; existing untouched hot-path code MAY still allocate (catalogued in [`docs/architecture/gc-discipline.md`](docs/architecture/gc-discipline.md) § "Known untouched allocators") and review is opt-in via `/allocation-audit`. This is the codification of the 2026-05-25 GC-discipline sweep — see [docs/LESSONS.md](docs/LESSONS.md) 2026-05-25 and [docs/architecture/gc-discipline.md](docs/architecture/gc-discipline.md).
...
**When required (invariant #8 corollary).** Any PR that changes `src/client/net/`, `src/core/prediction/`, `src/core/physics/`, the client render loop, snapshot decode/interpolate (`swarmWireFormat`, `interpolateSwarmPose`), mount aim (`WeaponMountController` / `tickLocalMountAim`), or `SectorRoom`'s tick/snapshot path. If the change can plausibly move `data-pred-stats`, the netgate is part of "done."

**How to run.**

- `pnpm e2e:netgate` — defaults: baseline `origin/main`, head = working tree, primary mobile profile (≈120 ms RTT ±60 ms, 0 % drop), 4 interleaved reps `A/B/A/B/A/B/A/B`.
- The driver (`tests/netgate/run-netgate.ts`) creates two git worktrees under `.claude/worktrees/netgate-{baseline,head}` (incremental — preserves `node_modules` across runs), boots two `vite` dev servers with HMR disabled on both, starts a per-arm HTTP+WS latency proxy, and runs `tests/e2e/netcode-health.spec.ts` interleaved on each. ~6–8 min for an acceptance run.
