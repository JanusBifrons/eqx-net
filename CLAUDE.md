...
**Auto-retry ONCE, then BELIEVE it (policy 2026-06-15 — corrects the earlier "never retry" purism).** The netgate is a relative measurement and is variance-prone on contended hosted runners; empirically every recurring RED cleared on a fresh-runner re-run, so the "never retry, only characterise" stance was bad advice in practice. The `netgate-retry` job now **auto-re-runs the gate exactly once on a fresh runner** (`run_attempt == '1'` guard — `rerun-failed-jobs`). This is NOT retry-until-green: a failure that **survives the retry (attempt ≥ 2) is LEGITIMATE.** Treat it as a real regression — **investigate the netcode; do NOT report it as a "host flake" or "the box."** Deflecting a surviving failure to the host is the exact reflex this policy exists to stop. Widening a margin to silence a flake is still forbidden (mechanism, not margin). When a surviving RED really is environmental, the fix is a *mechanism* one (a quieter/self-hosted runner, a baseline-sanity liveness precondition), surfaced as such — never a shrug.
