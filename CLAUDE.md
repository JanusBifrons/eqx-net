...
| `src/server/` | `colyseus`, `@colyseus/schema`, `@colyseus/ws-transport`, `express`, `zod`, `better-sqlite3`, `pino` + `pino-pretty` + `pino-roll` (rolling-NDJSON gameplay audit log — see `docs/architecture/gameplay-audit-log.md`), `web-push` (Web Push notifications, e.g. "your base is under attack" — see `docs/architecture/web-push.md`). Optional Phase 9 (multi-VM only): `@colyseus/redis-driver`, `@colyseus/redis-presence`. | Authoritative simulation host, persistence, lag comp, backpressure, orchestration |
